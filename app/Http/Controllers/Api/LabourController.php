...
class LabourController extends Controller
{
    ...
    public function update(Request $request, $id)
    {
        ...
        $request->validate([
            ...
            // 📋 Section 1 du Guide OMS — acceptés à la mise à jour
        ...
        // ✅ Mise à jour sélective : seuls les champs présents dans la requête
        // sont modifiés — les absents ne sont jamais écrasés avec null.
        $fields = [
            'start_time', 'end_time', 'status',
            'labor_onset', 'active_phase_diagnosis_at',
            'membranes_ruptured', 'membranes_rupture_at',
        ...
        if (! empty($toUpdate)) {
            $labour->update($toUpdate);
        }

        return response()->json($labour->fresh());
    }
